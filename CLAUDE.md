...
scp ml_training/hyperparam_tuning.py varant@<training-host>:~/ml_training/

# Start training in background (persists after SSH disconnect)
ssh <user>@<training-host> "cd ~/ml_training && source .venv/bin/activate && nohup python hyperparam_tuning.py > tuning.log 2>&1 &"

# Check if training is running
ssh <user>@<training-host> "ps aux | grep python | grep -v grep"

# Monitor training progress
ssh <user>@<training-host> "tail -50 ~/ml_training/tuning.log"

# Check GPU utilization
ssh <user>@<training-host> "nvidia-smi"

# View hyperparameter tuning results
ssh <user>@<training-host> "cat ~/ml_training/models/tuning_results.json"
```
...
scp varant@<training-host>:~/ml_training/models/tuning_best_*.pth ml_training/models/
scp varant@<training-host>:~/ml_training/models/tuning_tokenizer_*.pkl ml_training/models/
scp varant@<training-host>:~/ml_training/models/tuning_results.json ml_training/models/
```
